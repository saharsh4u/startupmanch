...
create or replace function public.fetch_pitch_feed(tab text, max_items int default 20)
returns table (
  pitch_id uuid,
  startup_id uuid,
  startup_name text,
...
as $$
  ...
  order by
    case when tab = 'fresh' then p.created_at end desc,
    case when tab <> 'fresh' then coalesce(st.score, 0) end desc,
    p.created_at desc
  limit max_items;
$$;
